...
void ILI9341_Init ( void )
{
	// ILI9341_GPIO_Config ();
	// ILI9341_FSMC_Config ();
	//FSMC 与 GPIO 初始化已经在 main.c 的 MX_FSMC_Init() 中完成
...
//digitalX是直接操作寄存器的宏定义，为了自己看得懂，可以把它们改成HAL库函数
...
void LCD_print(uint16_t line, char * format, ...)
{
  char dispbuffer[256];
  va_list args;
  va_start(args, format);
  vsprintf(dispbuffer, format, args);
  va_end(args);
  ILI9341_DispStringLine_EN(LINE(line), dispbuffer);
}
